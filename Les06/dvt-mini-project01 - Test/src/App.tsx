...
function App() {
  // State để quản lý việc hiển thị form
  const [showForm, setShowForm] = useState(false);

  // Function để toggle form giống như jQuery trong HTML
      ...
      <section className='container mt-4'>
        <div className='row my-2'>
          <div className='col-12 col-md border'>
            <Control onAddStudent={handleToggleForm} />
            <hr />
            {/* <StudentList /> */}
            <StudentList1 students={students} />
          </div>
          <div 
            className={`col-md-5 col-12 border ms-0 mt-2 ms-sm-2 mt-sm-0 ${showForm ? '' : 'd-none'}`} 
            id='frmForm'
          >
            <StudentForm />
          </div>
